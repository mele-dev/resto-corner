          ...
          try {
            ...
            if (queryResponse.isCompleted) {
              ...
              if (remainingTime !== null && remainingTime <= 0 && queryResponse.statusCode === 10) {
                console.warn(`🔄 [TablesView] ResponseCode 10 con RemainingExpirationTime = 0 detectado. Enviando reverso automáticamente...`);
                setPosStatusMessage(`⚠️ ${fullMessage} - Tiempo agotado. Enviando reverso...`);
              ...
              if (attempts >= maxAttempts) {
                console.error('⏱️ [TablesView] Tiempo de espera excedido para la transacción POS. Enviando reverso...');
                clearInterval(pollInterval);
                setIsPOSWaitingModalOpen(false);
                ...
                try {
                  ...
                  console.error('❌ [TablesView] Error al enviar reverso después de timeout:', reverseError);
              ...
              // Después del mínimo de intentos, si sigue sin resolverse, dar error y enviar reverso
              if (attempts >= maxAttempts) {
                console.error('⏱️ [TablesView] Tiempo de espera excedido para la transacción POS (estado desconocido). Enviando reverso...');
                clearInterval(pollInterval);
                setIsPOSWaitingModalOpen(false);
            ...
            if (attempts >= 10) { // Después de 10 intentos fallidos, considerar que no hay respuesta
              console.warn('🔄 [TablesView] Múltiples errores de conexión detectados. Enviando reverso...');
              clearInterval(pollInterval);
              setIsPOSWaitingModalOpen(false);
              ...
              try {
                ...
                console.error('❌ [TablesView] Error al enviar reverso después de error de conexión:', reverseError);
